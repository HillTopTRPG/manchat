class Api::V1::SynchronizeRecord < ApplicationRecord
  ...
  after_update -> {
    ActionCable.server.broadcast("room_#{self.room_uuid}", { type: "update-data", table: self.class.table_name, data: self.attributes.reject { |key| key == 'password' }, changes: self.previous_changes })
  }
  after_destroy -> {
