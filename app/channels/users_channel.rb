...
class UsersChannel < ApplicationCable::Channel
  def subscribed
    ...
    Api::V1::Chat.make_system_message(params[:room_uuid], "#{user.name || ''}が入室しました。") unless user.nil?
  end

  def unsubscribed
    Api::V1::UsersChannelSubscriber.unsubscribed(params)
    user = Api::V1::User.find_by(room_uuid: params[:room_uuid], uuid: params[:user_uuid])
    Api::V1::Chat.make_system_message(params[:room_uuid], "#{user.name || ''}が退室しました。") unless user.nil?
  end
